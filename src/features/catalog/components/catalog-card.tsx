...
import { canPurchase, getCatalogTransitionName, type CatalogItem } from "../domain/catalog-item";

export function CatalogCard({ item, index }: { item: CatalogItem; index: number }) {
  const purchasable = canPurchase(item);
  const style = { "--card-order": Math.min(index, 12) } as CSSProperties;
      ...
      <article className="catalog-card" style={style}>
        ...
        >
          ...
          <div className="card-copy">
            <p className="item-meta">{getCatalogCategory(item)} · {item.rarity}</p>
            <h2>{item.name}</h2>
            <div className="prices">
              <span className="vbucks">◉ {item.finalPriceVbucks.toLocaleString("es-MX")}</span>
              <strong>{item.priceMxn === null ? "Consultar" : formatMxn(item.priceMxn)}</strong>
            </div>
          </div>
        </Link>
      </article>
